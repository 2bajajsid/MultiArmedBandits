...
from e2d.arms.cauchy_arm import Cauchy_Arm
from e2d.model_class.model import Model
import math

class Cauchy_Model_Class(Model):
    def __init__(self, Delta, K=2, scale = 0.5):
        ...
        for i in range(0, K):
            if (i == d):
                self.arms.append(Cauchy_Arm(np.random.normal(loc = Delta, 
                                                       scale = 0.05)))
            else:
                self.arms.append(Cauchy_Arm(np.random.normal(loc = 0,
                                                           scale = 0.05)))   
