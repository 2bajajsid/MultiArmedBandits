...
from e2d.arms.t_arm import Student_t_Arm
from e2d.model_class.model import Model
import math

class Student_t_Model_Class(Model):
    def __init__(self, Delta, K=2, scale = 0.5):
        ...
        for i in range(0, K):
            if (i == d):
                self.arms.append(Student_t_Arm(np.random.normal(loc = Delta, 
                                                                scale = 0.05), 
                                               scale))
            else:
                self.arms.append(Student_t_Arm(np.random.normal(loc = 0, 
                                                                scale = 0.05), 
                                               scale))   
