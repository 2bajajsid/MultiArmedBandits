...
class DropOutLog_FI(Bandit_Algorithm_FI):
    def __init__(self, data_generating_mechanism, p):
        super().__init__()
        ...
        self.p = p
        self.__label = "Drop Out: O(log(t)^{prob: 0.2f})".format(prob = self.p)
    ...
    def get_arm_to_pull(self, losses, t):
        if (t < self.exploration_phase_length):
            ...
            uniform_sample = list(random.choice(t, size = math.floor(math.log(t) ** self.p), replace=False))
            copied_history[:, uniform_sample] = np.zeros(shape = (self.K, math.floor(math.log(t) ** self.p)))
            arm_estimates_current_round = np.mean(copied_history, axis = 1)
            return np.argmin(arm_estimates_current_round)
        
class DropOutLog_PI(Bandit_Algorithm_PI):
    def __init__(self, data_generating_mechanism, p):
        super().__init__()
        self.K = data_generating_mechanism.get_K()
        self.T = data_generating_mechanism.get_T()
