...
class Full_Information_Game(Game):
    ...
    def simulate_one_run(self, hyperparameter):
       ...
       for t in range(1, self.data_generating_mechanism.get_T() + 1):
            ...
            p_t = self.bandit_algorithm.get_arm_distribution()
            ...
            if t % 100 == 1:
                print(self.bandit_algorithm.label)
                print(t)
            
            current_loss[t-1] = np.dot(p_t, r_t) if self.store_rewards else np.dot(p_t, 1 - r_t)
